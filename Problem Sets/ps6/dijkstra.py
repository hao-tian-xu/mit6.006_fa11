...
class NodeDistancePair(object):
    ...
    def __init__(self, node, distance, passed):
...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        """
# version 0
    #     source_pair = NodeDistancePair((source,), weight(source, destination), 0)
    #     visited = 1
    #     expanded = set()
    #     queue = PriorityQueue()
    #     queue.insert(source_pair)

    #     while queue.heap != [None]:
    #         pair = queue.extract_min()
    #         path, dist, passed = pair.node, pair.distance, pair.passed
    #         node = path[0]
    #         if node in expanded: continue
    #         if node is destination:
    #             return (self._expand(path), visited)
    #         expanded.add(node)
    #         for adj_node in node.adj:
    #             if adj_node in expanded: continue
    #             adj_pair = NodeDistancePair(
    #                 (adj_node, path), 
    #                 passed + weight(node, adj_node) + weight(adj_node, destination), 
    #                 passed + weight(node, adj_node)
    #             )
    #             queue.insert(adj_pair)
    #             visited += 1
...
# version 1
        ...
        while queue.heap != [None]:
            pair = queue.extract_min()
            node, dist, passed = pair.node, pair.distance, pair.passed
            if node in expanded: continue
            if node is destination:
                return (self._Path(pair, parent), visited)
            expanded.add(node)
            for adj_node in node.adj:
                if adj_node in expanded: continue
                adj_pair = NodeDistancePair(
                ...
                queue.insert(adj_pair)
                parent[adj_pair] = pair
                visited += 1

    def _Path(self, pair, parent):
        path = [pair.node]
        while parent[pair] != None:
            pair = parent[pair]
            path.append(pair.node)
        path.reverse()
        return path
...
# version 2
    ...
    #     expanded = set()
    #     queue = PriorityQueue()
    #     queue.insert((weight(source, destination), 0, (source,)))
    #     while queue.heap != [None]:
    #         dist, passed, path = queue.extract_min()
    #         node = path[0]
    #         if node in expanded: continue
    #         if node is destination:
    #             return (self._expand(path), visited)
